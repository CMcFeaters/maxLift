...
def addRecW(form):
	#adds an individual record to our table, then commits it, web version
	#form is an array generated by the html form
	nType=form['nType']
	nMonth=form['nMonth']
	nDay=form['nDay']
	nYear=form['nYear']
	nmaxReps=form['nmaxReps']
	nmaxWeight=form['nmaxWeight']
	session.add(Lift(nType,nMonth,nDay,nYear,nmaxReps,nmaxWeight)) #this is still an outstanding change 
	session.commit()
	
def addRec():
	#adds an individual record to our table, then commits it, text version
